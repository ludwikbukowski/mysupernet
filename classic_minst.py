...
parser.add_argument('--stopat1', '-g', help="numer of epochs to iterate", type= float, default=0.99)
parser.add_argument('--stopat2', '-l', help="numer of epochs to iterate", type= float, default=0.99)
parser.add_argument('--stopat3', '-j', help="numer of epochs to iterate", type= float, default=0.99)
...
x_test = X_train[part_percentage: len(X_train)]
y_test = y_train[part_percentage: len(y_train)]

x_train = X_train[0:part_percentage]
y_train = y_train[0:part_percentage]
...
if(trainroot != 0):
    print("-------------------------------")
    print("Training Root...")
    checkpoint = keras.callbacks.ModelCheckpoint(filepath='minst_{epoch:02d}_tmp.h5', save_best_only=False, monitor='val_acc', mode='max',
...
if(trainsubs!=0):
    model = load_model("minst_" + str(epochs1) + "_tmp.h5")
    subs = []
